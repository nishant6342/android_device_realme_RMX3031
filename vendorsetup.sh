...
if ! [ -a $VT ]; then git clone --depth=1 https://github.com/nishant6342/vendor_realme_RMX3031 -b RMUI4-OSS vendor/realme/RMX3031
fi
...
if ! [ -a $MTK_SEPOLICY ]; then git clone --depth=1 https://github.com/Project-Elixir/device_mediatek_sepolicy_vndr -b UNO device/mediatek/sepolicy_vndr
fi
PARTS=packages/apps/RealmeParts/parts.mk
if ! [ -a $PARTS ]; then git clone --depth=1 https://github.com/nishant6342/packages_apps_RealmeParts -b UNO packages/apps/RealmeParts
fi
...
if ! [ -a $FW ]; then git clone --depth=1 https://github.com/nishant6342/vendor_realme_RMX3031-firmware vendor/realme/RMX3031-firmware
fi
...
if ! [ -a $CLANG17 ]; then git clone --depth=1 https://gitlab.com/projectelixiros/android_prebuilts_clang_host_linux-x86_clang-r487747 -b Tiramisu prebuilts/clang/host/linux-x86/clang-r487747
fi
