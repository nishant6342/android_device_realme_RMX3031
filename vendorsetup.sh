...
if ! [ -a $PARTS ]; then git clone --depth=1 https://github.com/nishant6342/packages_apps_RealmeParts -b Tiramisu packages/apps/RealmeParts
fi
POCKET=packages/apps/PocketMode/pocket_mode.mk
if ! [ -a $POCKET ]; then git clone --depth=1 https://github.com/nishant6342/packages_apps_PocketMode -b Tiramisu packages/apps/PocketMode
fi
